...
"yanarhs_f3_analysis"
"where fstats txt is"
(ft_fix="G:\\hmiwa\\m_20211215\\07_yana_saami3\\git\\analysis_yanarhsf3_2\\localRfstats\\results\\yanarhs_f3_analysis")
"where picture will be saved"
(pn_fix="G:\\hmiwa\\m_20211215\\07_yana_saami3\\git\\analysis_yanarhsf3_2\\localRfstats\\results\\picture\\yanarhs_f3_analysis")
"where .ind file is"
...
for(i in c("",2)){
  ...
  for(ah in c("ch1.mini01_lessmemory","ch1.mini02_lessmemory","ch1.mini03_lessmemory","ch1.mini04_lessmemory","ch1.mini05_lessmemory")){
    ...
    ( outputname <- sprintf("%s%s_%s_3pop.png",pn_fix,i,ah) )
    
    resultf3_all <- NULL
    
    for(ppops in c("FIN","GBR","JPT")){
      resultf3 <- NULL
      ( filename <- sprintf("%s%s_%s_%s.txt",ft_fix,i,ah,ppops) ) #part3 or 4
      ...
      resultf3.dat <- data.frame(resultf3.dat,popsname=ppops)
      
      resultf3_all <- rbind(resultf3_all,resultf3.dat)
    }
    ...
    g <- ggplot(resultf3_all,aes(x=f3,y=sample,color=pop,shape=popsname)) + 
      #g <- ggplot(resultf3.dat,aes(x=f3,y=sample)) + 
      geom_point(size=5) +
      geom_errorbarh(data=resultf3_all,aes(y=sample,xmin=f3-se,xmax=f3+se,color=pop),height=0.3) +
